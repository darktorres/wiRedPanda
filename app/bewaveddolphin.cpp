    ...
    for (int column = 0; column < m_model->columnCount(); ++column) {
        ...
        if (m_isTemporalSimulation) {
            for (int delta = 0; delta < 8; ++delta) {
                ...
                for (auto *output : std::as_const(m_outputs)) {
                    for (int port = 0; port < output->inputSize(); ++port) {
                        m_output_values[row][delta] = static_cast<int>(output->inputPort(port)->status());
                        ++row;
                    }
                }
            ...
            for (const auto &output : m_output_values) {
                ...
                if (!lastIndex.isValid()) {
                    createComposedWaveFormElement(row, column, output, std::nullopt);
                } else {
...
void BewavedDolphin::createTemporalSimulationElement(const int row, const int col, QPixmap composedWaveForm, const QString hex)
{
    const auto index = m_model->index(row, col);
